...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        $sql = "SELECT name, email, address, class_id FROM student WHERE id = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':id', $student_id, PDO::PARAM_INT);
        $stmt->execute();
        $student = $stmt->fetch(PDO::FETCH_ASSOC);

        // If student not found, handle error or redirect
        if (!$student) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // If there are no errors, update the database using PDO
        ...
        <main class=" sm:p-10 space-y-6">
            ...
            <div class="max-w-md w-full mx-auto mt-8 space-y-8 p-10 bg-white rounded-xl shadow-lg z-10">
                <div class="grid  gap-8 grid-cols-1 ">
                    ...
                    <!-- HTML Form -->
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?id=" . urlencode($_GET['id'])); ?>" method="post" enctype="multipart/form-data" class="form space-y-2">
                        <div class="relative z-0 w-full mb-5">
                            <label for="name" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-gray-600">Enter
                                name</label>
                        ...
                        <div class="relative z-0 w-full mb-5">
                            <label for="email" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-gray-600">Enter
                                email address</label>
                        ...
                        <div class="relative z-0 w-full mb-5">
                            <label for="address" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-gray-600">Enter
                                address</label>
                        ...
                        <div class="relative z-0 w-full mb-5">
                            <label for="class_id" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-gray-600">Select
                                class</label>
                        ...
                        <div class="relative z-0 w-full mb-5">
                            <label for="image" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-gray-600">Company
                            ...
                            <div class="flex items-center py-6">
                                <label for="image" class="cursor-pointer">
                                    ...
                                    <input type="file" name="image" id="image" class="hidden" accept=".jpg, .png">
                                </label>
